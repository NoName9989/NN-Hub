...
local NN HubFolder = "NN Hub"
local ConfigurationFolder = NN HubFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'NN Hub Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(NN HubFolder) then
				if isfile and isfile(NN HubFolder..'/settings'..ConfigurationExtension) then
					file = readfile(NN HubFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"NN Hub Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"NN Hub Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('NN Hub had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("NN Hub", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify NN Hub developers at sirius.menu/discord.")
				end
			end)
...
local NN Hub = useStudio and script.Parent:FindFirstChild('NN Hub') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if NN Hub:FindFirstChild('Build') and NN Hub.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('NN Hub | Build Mismatch')
		print('NN Hub may encounter issues as you are running an incompatible interface version ('.. ((NN Hub:FindFirstChild('Build') and NN Hub.Build.Value) or 'No Build') ..').\n\nThis version of NN Hub is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, NN Hub = NN Hub, useStudio and script.Parent:FindFirstChild('NN Hub') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

NN Hub.Enabled = false

if gethui then
	NN Hub.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(NN Hub)
	NN Hub.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	NN Hub.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == NN Hub.Name and Interface ~= NN Hub then
			Interface.Enabled = false
			Interface.Name = "NN Hub-Old"
		end
	end
elseif not useStudio then
...
if NN Hub.AbsoluteSize.X < minSize.X and NN Hub.AbsoluteSize.Y < minSize.Y then
...
local Main = NN Hub.Main
local MPrompt = NN Hub:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = NN Hub:FindFirstChild('Drag')
...
NN Hub.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/NN Hub/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = NN Hub.Notifications

local SelectedTheme = NN HubLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = NN HubLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	NN Hub.Main.BackgroundColor3 = SelectedTheme.Background
	NN Hub.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	NN Hub.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	NN Hub.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	NN Hub.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	NN Hub.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	NN Hub.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		NN Hub.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		NN Hub.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(NN Hub:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("NN Hub | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("NN Hub | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('NN Hub had an issue decoding the configuration file, please try delete the file and reopen NN Hub.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(NN HubLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("NN Hub | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--NN HubLibrary:Notify({Title = "NN Hub Flags", Content = "NN Hub was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(NN HubLibrary.Flags) do
...
function NN HubLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('NN Hub | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			NN HubLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show NN Hub'.", Duration = 7, Image = 4400697855})
		else
			NN HubLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.rayfieldOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(NN HubFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('NN Hub Settings', 0, true)

	if TabList['NN Hub Settings'] then
		TabList['NN Hub Settings'].LayoutOrder = 1000
	end

	if Elements['NN Hub Settings'] then
		Elements['NN Hub Settings'].LayoutOrder = 1000
	end
...
function NN HubLibrary:CreateWindow(Settings)
	if NN Hub:FindFirstChild('Loading') then
		if getgenv and not getgenv().rayfieldCached then
			NN Hub.Enabled = true
			NN Hub.Loading.Visible = true

			task.wait(1.4)
			NN Hub.Loading.Visible = false
		end
	end
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				NN HubLibrary:Notify({Title = 'Build Mismatch', Content = 'NN Hub may encounter issues as you are running an incompatible interface version ('.. ((NN Hub:FindFirstChild('Build') and NN Hub.Build.Value) or 'No Build') ..').\n\nThis version of NN Hub is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(NN HubFolder) then
		makefolder(NN HubFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "NN Hub"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "NN Hub Interface Suite" then
		LoadingFrame.Version.Text = "NN Hub UI"
	end
	...
	if not Settings.DisableNN HubPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				NN HubLibrary:Notify({
					Title = "NN Hub Interface",
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(NN HubFolder.."/Discord Invites") then
			makefolder(NN HubFolder.."/Discord Invites")
		end

		if isfile and not isfile(NN HubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(NN HubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"NN Hub RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(NN HubFolder.."/Key System") then
			makefolder(NN HubFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("NN Hub | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with NN Hub specific development.')
				end
			end
		end
		...
		if isfile and isfile(NN HubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(NN HubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(NN HubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						NN HubLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("NN Hub | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with NN Hub specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= NN HubLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					NN HubLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			NN Hub.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("NN Hub | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with NN Hub specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					NN HubLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("NN Hub | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with NN Hub specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					NN HubLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("NN Hub | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with NN Hub specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					NN HubLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= NN HubLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("NN Hub | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with NN Hub specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						NN HubLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("NN Hub | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with NN Hub specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					NN HubLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			NN HubLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			NN HubLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('NN Hub had an issue creating settings.') end
...
function NN HubLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function NN HubLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function NN HubLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['NN Hub Settings'])
		end)
	end)
...
function NN HubLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				NN HubLibrary:Notify({Title = "NN Hub Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			NN HubLibrary:Notify({Title = "NN Hub Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('NN Hub Configurations Error | '..tostring(result))
			NN HubLibrary:Notify({Title = "NN Hub Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = NN HubLibrary:CreateWindow({
		Name = "NN Hub Example Window",
		LoadingTitle = "NN Hub Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using NN Hub may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like NN Hub to get the key from
	...
	--NN HubLibrary:Notify({Title = "NN Hub Interface", Content = "Welcome to NN Hub. These - are the brand new notification design for NN Hub, with custom sizing and NN Hub calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(NN HubLibrary.Theme) do
...
task.delay(4, function()
	NN HubLibrary.LoadConfiguration()
